...
class Board:
    ...
    def first_empty_square(self) -> Optional[tuple[int, int]]:
        for row in range(8):
            for col in range(8):
                if self._squares[row][col] is None:
                    return (row, col)
        return None

    def place_piece(self, piece: Piece) -> tuple[int, int]:
        square = self.first_empty_square()
        if square is None:
            raise ValueError("Board has no empty squares for captured pieces")
    ...
    @classmethod
    def from_list(cls, pieces: list[dict]) -> Board:
        board = cls()
        for item in pieces:
            row = item.get('row')
            col = item.get('col')
            ...
            if not isinstance(row, int) or not isinstance(col, int):
                raise ValueError('Each piece must define integer row and col')
            if not cls.in_bounds(row, col):
                raise ValueError(f'Piece position out of bounds: {row}, {col}')
            ...
            if board.get(row, col) is not None:
                raise ValueError(f'Duplicate piece position: {row}, {col}')

            board.set(row, col, Piece(color, piece_type))
